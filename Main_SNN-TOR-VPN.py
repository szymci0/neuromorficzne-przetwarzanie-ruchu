...
'Dataset'                 : "ISCX_nonVPN",       # MNIST or FashionMNIST or CIFAR10 or ISCX or ISCX_nonVPN or ISCX_VPN or ISCX_Tor
...
def train(model, params, optimizer, train_dataloader, valid_dataloader, last_checkpoint=None, scheduler=None):
    ...
    for e in range(User_params['nb_epochs']):
        ...
        print(f"Epoch {e+1}: Execution time={end_time-start_time}")
        ...
        for i, loss in enumerate(reg_loss):
            ...
            print(f"Layer {i}: reg loss={mean_reg_loss}")

        for i, l in enumerate(snn.layers[:-1]):
            try:
                print(f"Layer {i}: average number of spikes={User_params['nb_steps'] * l.spk_rec_hist.mean()}")
            except:
                print(f"Layer {i}: No spikes exist in this layer!")
...
def compute_and_print_score_categories(confusion_matrix):
    ...
    for i in range(len(categ_list)):
        ...
        print(f"{categ_name[i]} : Re = {(Re*100)}% ; Pr = {Pr*100}  ; Ac = {Ac*100}%")
    ...
    for ind in range(User_params['nb_outputs']):
        ...
        print(str(utils.key(label_dct,ind)) + " (" + str(ind) + ")" + f"Re = {(Re*100)}% ; Pr = {Pr*100}  ; Ac = {Ac*100}%")
