...
def load_dataset(User_params, root_dir, root_dir_data, np_dtype) :
    if User_params['Project'] == "Image":
        ...
        try:
            ...
            print(f"Error while trying to create the main train_dataset/train_dataloader, creating another one similar to validation ...{e}")
    ...
    def __init__(self, User_params, data_root, label_dct, mode, bins, shift):
        ...
        for root, dirs, files in os.walk(data_root):
            for filename in files:
                ...
                if (self.User_params['Problem']) == "All" or User_params['Problem']=="All_with_encryption_feature":
                    label_name = filename.split("_")[2].capitalize()
                    if label_name == "File":
                        label_name = "FileTransfer"
                    if label_name == "Voip":
                        label_name = "VOIP"
                    if "vpn" in filename.split("_")[-4].lower():
                        label_name = label_name + "_VPN"
                    elif "tor" in filename.split("_")[-4].lower():
                        label_name = label_name + "_Tor"
                    else:
                        label_name = label_name + "_nonVPN"
        ...
        if self.mode == "train":
            ...
            for id, label in enumerate(np.unique(self.labels)):
                weights_dict[label] = label_weights[id]
            self.weights = torch.DoubleTensor([weights_dict[label] for label in self.labels])
            print(self.weights)
