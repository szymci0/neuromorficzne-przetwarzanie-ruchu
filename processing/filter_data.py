...
def data(session):
    ...
    IP = session.split("_")[-2].split("(")[1].replace(")", "").split("-")
    IP = (int(IP[0]), int(IP[1]))
    Port = session.split("_")[-1].split("(")[1].replace(")", "").split("-")
...
def splitting(pairs):
    ...
    for t in range(int(time_max / DELTA_T - TPS / DELTA_T) + 1):
        ind_times1 = [i for i in range(len(times1)) if
                      ((times1[i] >= t * DELTA_T) and (times1[i] <= (t * DELTA_T + TPS)))]
        ind_times2 = [i for i in range(len(times2)) if
                      ((times2[i] >= t * DELTA_T) and (times2[i] <= (t * DELTA_T + TPS)))]
        ...
        split_dict.update({name: {"1_Time_of_arrival": Time1, "1_Packet_size": Size1, "2_Time_of_arrival": Time2,
                                  "2_Packet_size": Size2}})
    ...
    for root, dirs, files in os.walk("./data"):
        ...
        for file in files:
            ...
            for row in reader:
                ...
                name = label + "_" + Raw[0].replace("_", "") + "_(" + str(IP_adresses[row[1]]) + "-" + str(
                    IP_adresses[row[3]]) + ")_(" + str(row[2]) + "-" + str(row[4]) + ")"
